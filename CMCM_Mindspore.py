...
if __name__ == '__main__':
    ...
    x1 = np.random.uniform(-1, 1, (2, 3, 224, 224)).astype(np.float32)
    x1 = ms.Tensor(x1)
    x2 = np.random.uniform(-1, 1, (2, 3, 224, 224)).astype(np.float32)
    x2 = ms.Tensor(x2)
    x3 = np.random.uniform(-1, 1, (2, 3, 224, 224)).astype(np.float32)
    x3 = ms.Tensor(x3)
    x4 = np.random.uniform(-1, 1, (2, 250, 300)).astype(np.float32)
    x4 = ms.Tensor(x4)
    net = CMNet()
    y = net(x1,x2,x3,x4)
    print(y[0].shape)